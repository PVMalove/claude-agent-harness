...
6. After approval, open the PR/MR with the tracker CLI and `--body-file <path>`. Delete the `.harness/scratch/tmp/` body file only after that command succeeds; on failure, retain it for retry. If the ticket carries `task-report::required`, publish its completion report on the ticket, unless the developer asked to skip it.
...
8. **Unblock dependencies:** After the ticket is successfully closed, check if any other open tickets were blocked by this one. For local tracker files (`.scratch/<feature>/issues/*.md` or `docs/tasks/*.md`), check for `**Blocked by:** #<ID>`. If a ticket is now fully unblocked, change its status from `status::blocked` to `status::ready` (via label or local file update).
