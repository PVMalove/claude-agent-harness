...
8. **Cleanup:** After confirming the PR is merged and the ticket is closed, clean up the local environment to prevent garbage accumulation. Switch out of the worktree if necessary, remove the worktree using `git worktree remove <path>`, and delete the local branch using `git branch -d <branch-name>`.
