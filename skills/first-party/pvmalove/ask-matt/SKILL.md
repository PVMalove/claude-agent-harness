...
3. **Branch — is this a multi-session build?**
   ...
   Either way, **`/implement`** builds each issue by driving **`/tdd`** internally — one red-green slice at a time — then asks the developer whether to run **`/code-review`**, a two-axis review (Standards + Spec) of the diff. After the report returns, or the developer declines review, it asks for approval to commit and push the current issue branch, then offers manual **`/to-pr`**. Reach for **`/tdd`** on its own when you just want to build a concrete behaviour test-first without a full spec, and **`/code-review`** on its own whenever you want to review a branch or PR against a fixed point.
