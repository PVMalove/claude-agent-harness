...
5. Output only the finished PR body markdown, directly as your response text — ready to pass verbatim to `gh pr create --body`. No preamble, no commentary outside the body itself. **Never write it to a file** (no `cat`/`printf`/heredoc, no Python `write_text`, no temp file + `cp`) — Bash is for gathering context in steps 1–3 only. If a Bash call for those steps errors, fix the actual cause and retry at most once; don't retry the same failing shape with a different quoting trick.
