...
5. Use the `Write` tool to save the finished PR body markdown only to the scratch path you were given; never use `docs/tasks/` or another location. Respond with only that path — no preamble, no commentary, no copy of the body itself. **Never use Bash to write file content** (no `cat`/`printf`/heredoc, no Python `write_text`/`-c` snippets, no temp file + `cp`) — Bash is for gathering context in steps 1–3 only. If a Bash call for those steps errors, fix the actual cause and retry at most once; don't retry the same failing shape with a different quoting trick.
