...
You're given: an issue number and the exact epic integration branch to diff against
(read from the child ticket or its parent epic), a target file path to write the body into, and the
last `qa-gate` result if one was run in this session. For an epic-less ticket, use `base_branch`
from `.harness/project.json`.
...
3. Gather context: `git diff <integration-branch>...HEAD` (three-dot, against the merge-base) and `git log <integration-branch>..HEAD --oneline`.
