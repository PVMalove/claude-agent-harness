...
- **Как работает:**
  ...
  4. **В этом репозитории**: если исходный issue — эпик (`workflow::specs`), каждый дочерний тикет линкуется к нему как native GitHub sub-issue (не лейблом); тикет, заблокированный другим ещё не закрытым тикетом той же декомпозиции, получает `workflow::blocked` вместо `workflow::ready` — `/implement` проверяет и снимает это состояние при старте (раздел 7, ADR в `docs/adr/`).
...
- **Как работает:**
  ...
  2. Если вызван со ссылкой на тикет (номер/URL/путь к локальному файлу) и тот несёт `workflow::blocked` — сначала проверяет блокеры и либо переводит в `workflow::ready`, либо отказывается стартовать, называя открытые блокеры (раздел 7). Начиная работу, ставит `workflow::in-progress`.
