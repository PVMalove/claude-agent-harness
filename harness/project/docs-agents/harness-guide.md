...
Тот же `/grilling`, плюс `/domain-modeling` поверх того же цикла раундов — ведение `CONTEXT.md`/ADR по ходу интервью (см. ниже). Для работы в существующем репозитории предпочтительнее `/grill-me` — раз оставляет бумажный след, это строго более сильный вариант того же интервью.
