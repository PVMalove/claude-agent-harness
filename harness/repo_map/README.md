...
5. **Ранжирование и бюджет.** С seeds файлы сортируются по расстоянию в графе через рёбра высокой и
   средней уверенности, без seeds — по входящей степени, затем по пути. Файлы добавляются по порядку,
   пока сериализованный JSON укладывается в `max_tokens`.
...
`tree_sitter_worker.py <install_dir>` читает из stdin `{"paths": {"<path>": "<base64>"}}` и пишет в
stdout `{"files": {"<path>": FileFacts}}`. Структура `FileFacts`: `parser_status`,
`signatures[{text, symbols[]}]`, `imports[{module, level, names[]}]`, `definitions[]`,
`references[]`. `parser_bundle._file_facts` отклоняет любой лишний или неверно типизированный ключ:
весь ответ тогда считается сбоем (`parser subprocess failed`). Worker автономен: он использует только
stdlib и пакеты `tree_sitter*` из `<install_dir>` и не импортирует harness.
...
- Кэш проверяется **до** поиска bundle, но его ключ уже включает идентичность выбранного lock,
  worker и wheels для текущего интерпретатора. Поэтому установка или замена bundle меняет ключ.
- Bundle находится и проверяется **до** чтения содержимого файлов. Деградировавший запуск читает
  только пути.
- Размер каждого файла проверяется через `git cat-file -s` до чтения. Файл больше `max_file_bytes`
  получает `too_large` и в worker не передаётся.
- Бинарные blob'ы (с байтом `\0`) в полном режиме исключаются из `files`. В режиме `minimal` они
  остаются в списке путей.
...
| --- | --- |
| `repo_map.py` | CLI, загрузка политики, отбор путей, граф, ранжирование, бюджет, кэш. |
| `contract.py` + `repo_map.schema.json` | Проверка выходного JSON без runtime-зависимостей: схема и согласованность `tier`/`parser`. |
| `parser_bundle.py` | Разбор и проверка lock, SHA-256 wheels и worker, offline-установка через `uv` под межпроцессной блокировкой, ограниченный запуск worker, проверка ответа `FileFacts`, provenance. |
...
(`python`, `js`, `go`, `java`, `csharp`); неизвестное расширение образует своё семейство. Рёбра
низкой уверенности попадают в вывод, но не влияют на ранжирование.
...
`repo_map.py`, `parser_bundle.py`, `contract.py`, схемы и worker, версию оценщика токенов, а в полном
режиме ещё и идентичность bundle. Полезная нагрузка защищена SHA-256 и повторно проверяется по схеме.
...
`scripts/build_parser_bundle.py` собирает smoke-bundle для CI из заранее скачанных wheels. Задача
`repo-map-bundle` в `.github/workflows/verify.yml` запускается матрицей из четырёх ветвей:
`python-ts-js`, `go`, `java`, `csharp`. Каждая ветвь проверяет SHA-256 wheels из
`.github/parser-bundle-wheels.txt`, собирает bundle и запускает свои тесты без `-k` и без пропусков.
Ветвь `python-ts-js` дополнительно гоняет строгий mypy для worker (`--strict --disallow-any-explicit`)
в отдельном окружении. Недоступный или подменённый wheel проваливает задачу. Основной mypy worker не
проверяет.
...
| Новый язык добавляется записью в `_LANGUAGE_EXTRACTORS` и грамматикой в lock | Ядро графа и ранжирования не меняется. | OCP (частично, см. долг) |
...
Долг выявлен ревью PR #296 и в этом релизе не исправлен:

- Таблица «расширение → грамматика» задана в трёх местах: `grammars[].extensions` в lock,
  словарь `parsers` в `tree_sitter_worker.main()` и `FAMILY_EXTENSIONS` в `repo_map.py`. Для полного
  OCP нужен один реестр языков.
- `parser_bundle.py` совмещает разбор lock, проверку, установку, запуск subprocess и контракт
  `FileFacts`. Кандидаты на выделение — модули `lock`, `install`, `runner` и `facts`.
- `_build_map` пересериализует весь payload на каждый добавленный файл: сложность квадратичная по
  числу файлов.
- `repo_map.py` жёстко зависит от модуля `parser_bundle`. Протокол `ParserBackend` позволил бы
  подменять backend в тестах без stub-bundle на диске.
