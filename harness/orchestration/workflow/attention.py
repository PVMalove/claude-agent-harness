...
from harness.orchestration.core.constants import (
    ATTENTION_STATE_FIELDS, CONTEXT_TELEMETRY_SOURCES, LIVE_DISPATCH_STATES, OPERATIONAL_REASON_CATEGORIES,
    TERMINAL_BATCH_STATES,
)
from harness.orchestration.core.utils import (
    CoordinatorError, JsonObject, _canonical, _moment, _non_empty, _now, _repo, _safe_id, _sanitise, _short,
    _silent_seconds,
)
from harness.orchestration.core.config import (
    _adaptive_continuation_policy, _attention_policy, _extension_names, _reject_sensitive, _role,
)
from harness.orchestration.ledger.ledger_ops import (
    _ledger_lock, _load_batch, _load_dispatch, _load_dispatch_status, _replace_record, _state_root,
)
from harness.orchestration.workflow.approval import _approval
from harness.orchestration.workflow.history import (
    _pinned_package_stale, _settled, _validate_batch_integrity,
)
...
def _require_no_attention(ledger: LifecycleLedger, repo: Path, root: Path, config: JsonObject, batch: JsonObject) -> None:
    ...
    if _apply_attention(config, batch, _attention_findings(repo, root, config, batch, utils._now()), utils._now()):
...
def _flag_stale_dispatch(ledger: LifecycleLedger, repo: Path, root: Path, dispatch: JsonObject, silent: int) -> None:
    ...
    if _apply_attention(core_config._config(repo), batch, [_stale_dispatch_finding(dispatch["dispatch_id"], silent)], utils._now()):
        _safe_id(batch["batch_id"], "batch")
        _replace_record(ledger, BatchRecord.from_dict(batch))
...
def attention_resolve(args: argparse.Namespace) -> JsonObject:
    ...
    with _ledger_lock(ledger):
        ...
        batch.setdefault("attention_events", []).append({
            "event": "resolved", "at": utils._now(), "keys": keys, "note": note, **approval,
        })
...
def record_context_pressure(args: argparse.Namespace) -> JsonObject:
    ...
    with _ledger_lock(ledger):
        ...
        record: JsonObject = {
            ...
            "level": level, "recorded_at": utils._now(), "source": source, "action_required": action_required,
