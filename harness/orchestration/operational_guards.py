...
TRANSITION_FIELDS = (
    "batch_id", "previous_dispatch_id", "previous_role", "reason_category", "next_role", "next_action",
    "purpose", "candidate_sha", "base_sha", "review_scope", "verification_commands", "context_package_id",
    "required_gates",
)
...
ATTENTION_REASONS = (
    "unknown-reason", "infrastructure-retry-repeated", "retry-queued-too-long", "stale-evidence", "stale-dispatch",
)
...
def build_transition(
    *, batch_id: str, previous_dispatch_id: str | None, previous_role: str | None, reason_category: str | None,
    next_role: str, next_action: str, purpose: str, candidate_sha: str | None, base_sha: str,
    review_scope: Sequence[str], verification_commands: Sequence[str], context_package_id: str | None,
    required_gates: Sequence[str],
) -> dict[str, object]:
    return {
        "batch_id": batch_id, "previous_dispatch_id": previous_dispatch_id, "previous_role": previous_role,
        "reason_category": reason_category, "next_role": next_role, "next_action": next_action, "purpose": purpose,
        "candidate_sha": candidate_sha, "base_sha": base_sha, "review_scope": list(review_scope),
        "verification_commands": list(verification_commands), "context_package_id": context_package_id,
...
def retry_idempotency_key(
    *, role: str, candidate_sha: str | None, base_sha: str, review_scope: Sequence[str], reason_category: str,
    verification_commands: Sequence[str],
) -> str:
    """One key per (role, candidate, base, scope, reason, verification): equal keys are the same request."""
    return _digest({
        "role": role, "candidate_sha": candidate_sha, "base_sha": base_sha, "review_scope": list(review_scope),
        "reason_category": reason_category, "verification_commands_digest": _digest(list(verification_commands)),
    })
...
def pressure_level(observed_tokens: int, context_limit: int, warning_ratio: float) -> tuple[int, str]:
    """``(warning_threshold, level)`` for an observed token count."""
    warning_threshold = round(context_limit * warning_ratio)
    return warning_threshold, level_for(observed_tokens, context_limit, warning_threshold)


def attention_finding(
    reason: str, subject: str, *, last_safe_action: str, recommended_human_action: str,
) -> dict[str, str]:
    ...
    if reason not in ATTENTION_REASONS:
        raise GuardError(f"unknown attention reason {reason!r}", remedy=f"use one of: {', '.join(ATTENTION_REASONS)}")
    return {
        "key": f"{reason}:{subject}", "reason": reason, "last_safe_action": last_safe_action,
