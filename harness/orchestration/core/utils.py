...
def _read_object(path: Path, label: str) -> JsonObject:
    ...
    if not isinstance(value, dict):
        raise CoordinatorError(
            f"{label} must be a JSON object", remedy=f"set {label} to a JSON object"
        )
    return value
...
def _strings(value: object, label: str, *, allow_empty: bool = False) -> list[str]:
    if (
        ...
        or not all(_non_empty(item) for item in value)
    ):
        raise CoordinatorError(
            f"{label} must be a list of non-empty strings",
            remedy=f"set {label} to a list of non-empty strings",
        )
    return list(value)
...
def _safe_id(value: object, label: str) -> str:
    if (
        ...
        or re.fullmatch(
            r"(?:batch|dispatch|risk|context-package|checkpoint)-[0-9a-f-]+", value
        )
        is None
    ):
        raise CoordinatorError(
            f"{label} is not a valid coordinator ID",
            remedy=f"use a valid coordinator-generated ID for {label}",
        )
    return value
...
def _moment(value: object, label: str) -> datetime:
    ...
    if parsed.tzinfo is None:
        raise CoordinatorError(
            ...
            remedy=f"include an explicit UTC offset (e.g. Z or +00:00) in {label}",
        )
    return parsed
...
def _silent_seconds(status: JsonObject) -> int:
    ...
    elapsed = datetime.now(UTC) - _moment(last, "dispatch heartbeat")
