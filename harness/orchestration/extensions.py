...
EXTENSION_KINDS = (
    "transport_health", "verification_environment_health", "retry_reason_classifier",
    "context_telemetry_provider", "human_notifier",
)
DEFAULT_EXTENSION = "none"
EXTENSION_NAME = re.compile(r"^[A-Za-z_][A-Za-z0-9_.-]*(:[A-Za-z_][A-Za-z0-9_]*)?$")
_METHOD = {
    "transport_health": "probe", "verification_environment_health": "probe", "retry_reason_classifier": "classify",
    "context_telemetry_provider": "observe", "human_notifier": "notify",
}
...
_REGISTRY: dict[str, dict[str, object]] = {kind: {DEFAULT_EXTENSION: _Inert()} for kind in EXTENSION_KINDS}


def _known_kind(kind: str) -> None:
    if kind not in _REGISTRY:
        raise ExtensionError(
            f"unknown extension kind {kind!r}", remedy=f"use one of: {', '.join(EXTENSION_KINDS)}",
        )
...
def register(kind: str, name: str, implementation: object) -> None:
    ...
    if name == DEFAULT_EXTENSION or EXTENSION_NAME.fullmatch(name) is None:
        raise ExtensionError(
            f"invalid extension name {name!r}", remedy="use a name that is not 'none' and matches [A-Za-z_][A-Za-z0-9_.-]*",
        )
...
def _resolve(kind: str, name: str) -> object:
    ...
    except Exception as exc:  # a project-supplied module may fail in any way; the run must fail closed
        raise ExtensionError(
...
def selected(config: Mapping[str, object]) -> dict[str, str]:
    ...
    if not isinstance(configured, dict):
        raise ExtensionError("orchestration extensions must be an object", remedy="set extensions to an object of interface names")
    for kind, name in configured.items():
        _known_kind(kind)
        if not isinstance(name, str) or (name != DEFAULT_EXTENSION and EXTENSION_NAME.fullmatch(name) is None):
            raise ExtensionError(
...
def verification_environment_health(name: str) -> VerificationEnvironmentHealth:
    return cast(VerificationEnvironmentHealth, _resolve("verification_environment_health", name))
