...
def build_parser(handlers: Any, defaults: Any) -> argparse.ArgumentParser:
    ...
    for name, handler, help_text in (
        ("status", handlers.ledger_status, "Report the selected lifecycle-ledger generation"),
        ("migrate", handlers.migrate_ledger, "Validate and migrate legacy state to current schema"),
        ("clean", handlers.clean_ledger, "Safely remove orphaned dispatch evidence to fix migration errors"),
    ):
    ...
    create.add_argument("--zone", default=defaults.DEFAULT_ZONE, help="backend zone; defaults to the whole repository")
    create.add_argument("--integration-ref", help="branch on origin this batch's base is fetched and pinned against; falls back to the project's base_branch for epic-less tasks")
    ...
    create.add_argument("--expected-file", action="append", help="planned changed path; repeated")
    create.add_argument("--expected-service", action="append", help="bounded context/service expected to change; repeated")
    create.add_argument("--expected-changed-lines", type=int, help="conservative expected diff size")
    create.add_argument("--expected-context-tokens", type=int, help="optional observed context estimate; never lowers the deterministic floor")
    create.set_defaults(handler=handlers.create_batch)
    batch_preflight = batch_commands.add_parser("preflight", help="reject an oversized ticket before creating a batch")
    ...
    context_package_register.add_argument("--max-package-tokens", type=int, help="optional stricter token ceiling")
    ...
    dispatch_rate_limited.add_argument("--retry-after-seconds", type=int, default=defaults.DEFAULT_RATE_LIMIT_RETRY_SECONDS)
    ...
    dispatch_resume.add_argument("--termination-reason", default="", help="runtime adapter termination reason; a recognized rate limit (rate_limit/rate-limit/429) authorizes the new worker session automatically")
    dispatch_resume.add_argument("--trigger", choices=sorted(defaults.PLANNED_TRIGGER_KINDS), help="planned-trigger kind; required unless --termination-reason is a recognized rate limit")
    dispatch_resume.add_argument("--measured-value", type=int, help="measured value for a context-limit/tdd-cycles/failure-log planned trigger, checked against the project's adaptive_continuation_policy threshold")
    dispatch_resume.add_argument("--file", help="continuation facts JSON restating remaining DoD/risks/blockers/dependencies; required for a planned-trigger continuation")
    ...
    dispatch_status_command.add_argument("--stale-after", type=int, default=defaults.DEFAULT_STALE_AFTER_SECONDS)
