...
class DispatchErrorInvariantTests(unittest.TestCase):
    ...
    def test_every_raise_site_passes_a_non_empty_remedy(self) -> None:
        ...
        sites = [
            ...
            and node.exc.func.id == "DispatchError"
        ]
...
class ValueValidatorTests(unittest.TestCase):
    ...
    def test_model_id_is_stripped_and_validated(self) -> None:
        self.assertEqual(
            adapter._validate_model_id(" claude-opus-4.1:x "), "claude-opus-4.1:x"
        )
    ...
    def test_sensitive_keys_are_rejected_at_any_depth(self) -> None:
        adapter._reject_sensitive_keys(
            {"a": [{"input_tokens": 1}, "x"], "n": None}, "cfg"
        )
        with self.assertRaises(adapter.DispatchError) as ctx:
            adapter._reject_sensitive_keys({"a": [{}, {"api_key": "x"}]}, "cfg")
        self.assertEqual(
            ctx.exception.message,
            "cfg.a[1] must not contain secret-shaped field 'api_key'",
        )
        with self.assertRaises(adapter.DispatchError) as ctx:
...
class OrcaPayloadTests(unittest.TestCase):
    ...
    def test_result_id_is_none_for_missing_blank_or_non_object_paths(self) -> None:
        ...
        self.assertIsNone(
            adapter._result_id({"result": {"task": "flat"}}, ("task", "id"))
        )
        self.assertIsNone(
            adapter._result_id({"result": {"task": {"id": "  "}}}, ("task", "id"))
        )
        self.assertIsNone(
            adapter._result_id({"result": {"task": {"id": 3}}}, ("task", "id"))
        )
...
class RunOrcaTests(unittest.TestCase):
    ...
    def test_returns_the_success_payload(self) -> None:
        ...
        self.assertEqual(
            adapter._run_orca(fake, ["a"]), {"ok": True, "result": {"x": 1}}
        )
    ...
    def test_unsuccessful_payloads_are_rejected(self) -> None:
        for body in ('print("[]")', "print('{\"ok\": false}')"):
            ...
            self.assertEqual(
                ctx.exception.message, "Orca returned an unsuccessful dispatch result"
            )
    ...
    def test_nonzero_exit_classifies_fallback_safety_by_error_code(self) -> None:
        ...
        for code, safe in cases.items():
            fake = self._fake(
                f'print(\'{{"error": {{"code": "{code}"}}}}\'); sys.exit(1)'
            )
...
class WriteRecordTests(unittest.TestCase):
    def test_writes_a_sorted_record_and_refuses_to_overwrite(self) -> None:
        ...
        self.assertEqual(
            ctx.exception.message, "refusing to overwrite an immutable dispatch record"
        )


class CandidateProfileTests(unittest.TestCase):
    ROLE: ClassVar[dict[str, list[str]]] = {"required_capabilities": ["write"]}
    ...
    def _candidates(
        self, plan: Mapping[str, object], preferred: object = None
    ...
    def test_fallback_chain_is_flattened_without_duplicates(self) -> None:
        ...
        self.assertEqual(
            self._candidates(plan), [("a", "m1", "high"), ("b", "m1", "high")]
        )
...
class GitHelperTests(unittest.TestCase):
    ...
    def test_resolved_commit_requires_the_full_sha(self) -> None:
        self.assertEqual(
            adapter._resolved_commit(self.repo, self.second.upper()), self.second
        )
    ...
    def test_candidate_files_with_and_without_a_base(self) -> None:
        self.assertEqual(
            adapter._candidate_files(self.repo, self.second, self.first), ["b.txt"]
        )
        self.assertEqual(
            adapter._candidate_files(self.repo, self.first, None), ["a.txt"]
        )
        with self.assertRaises(adapter.DispatchError) as ctx:
            adapter._candidate_files(self.repo, "0" * 40, None)
        self.assertEqual(
            ctx.exception.message, "cannot inspect the pinned candidate commit"
        )
...
class ParserTests(unittest.TestCase):
    def test_dispatch_arguments(self) -> None:
        args = adapter.parser().parse_args(
            ["dispatch", "--brief", "b.json", "--run", "r1"]
        )
        self.assertEqual(
            (args.repo, args.brief, args.run, args.records_dir),
            (".", "b.json", "r1", None),
        )
    ...
    def test_main_prints_an_error_and_exits_two_on_a_harness_error(self) -> None:
        ...
        self.assertIn(
            "REMEDY:", "".join(call.args[0] for call in stderr.write.call_args_list)
        )
