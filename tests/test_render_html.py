...
class FormatterTests(unittest.TestCase):
    ...
    def test_thousands_groups_ints_and_passes_other_values_through_escaped(self) -> None:
...
class PanelTests(unittest.TestCase):
    def test_cache_panel_is_missing_for_a_non_dict(self) -> None:
        ...
        self.assertIn(render_html.MISSING, render_html._cache_panel(render_html.MISSING))

    def test_cache_panel_reports_the_total_input(self) -> None:
        cache = {
            "cache_read_percent": 50.0, "cache_write_percent": 25.0, "fresh_percent": 25.0,
            "cache_read": 500, "cache_write": 250, "fresh": 250, "total_input": 1000,
        }
        markup = render_html._cache_panel(cache)
        self.assertIn("1"+NB+"000 токенов", markup)
        self.assertIn("чтение кеша", markup)

    def test_orchestration_panel_is_missing_for_a_non_ok_or_non_dict_value(self) -> None:
        self.assertIn(render_html.MISSING, render_html._orchestration_panel(None))
        markup = render_html._orchestration_panel({"status": render_html.MISSING, "reason": "no ledger"})
        self.assertIn("no ledger", markup)

    def test_orchestration_panel_lists_sessions_and_qa_rate(self) -> None:
        orchestration = {"status": "ok", "tickets": {"7": {
            "worker_sessions": [{"role": "developer", "sessions": 2, "restarts": []}],
            "qa_failure_rate": 0.5,
