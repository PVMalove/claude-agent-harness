    ...
    def test_every_raise_site_passes_a_non_empty_remedy(self) -> None:
        ...
        for site in sites:
            ...
            if isinstance(value, ast.Constant):
                self.assertTrue(
                    isinstance(value.value, str) and value.value.strip(),
                    f"line {site.lineno}",
                )
...
class RejectSensitiveTests(unittest.TestCase):
    ...
    def test_rejects_secret_shaped_key_with_nested_location(self) -> None:
        ...
        self.assertEqual(
            ...
            "config.outer contains secret-shaped field 'password'",
        )
    ...
    def test_rejects_secret_key_inside_list_with_index_location(self) -> None:
        ...
        self.assertEqual(
            ctx.exception.message,
            "config.items[1] contains secret-shaped field 'api_key'",
        )
...
class LoadRoleManifestTests(unittest.TestCase):
    ...
    def test_unreadable_manifest(self) -> None:
        ...
        self.assertEqual(
            ctx.exception.message, "role manifest 'role.md' cannot be read"
        )
    ...
    def test_missing_frontmatter(self) -> None:
        ...
        self.assertEqual(
            ctx.exception.message, "role manifest 'role.md' has no valid frontmatter"
        )
    ...
    def test_orphan_list_item(self) -> None:
        ...
        self.assertEqual(
            ctx.exception.message, "role manifest 'role.md' has an orphan list item"
        )
        self.assertIn(
            "put each '- item' line under a preceding 'key:' line", ctx.exception.remedy
        )
    ...
    def test_invalid_frontmatter_line(self) -> None:
        with self.assertRaises(contract.ContractError) as ctx:
            contract.load_role_manifest(self._load("---\nBad Line\n---\n"))
        self.assertEqual(
            ctx.exception.message, "role manifest 'role.md' has invalid frontmatter"
        )
...
class ResolveRuntimeNameTests(unittest.TestCase):
    def test_no_runtimes(self) -> None:
        ...
        self.assertEqual(
            ctx.exception.message, "assignment plan has no runtime assignments"
        )

    def test_single_runtime_needs_no_request(self) -> None:
        self.assertEqual(
            contract.resolve_runtime_name({"runtimes": {"claude": {}}}, None), "claude"
        )
    ...
    def test_unknown_explicit_request(self) -> None:
        ...
        self.assertEqual(
            ctx.exception.message, "role is not assigned to runtime 'other'"
        )
    ...
    return {
        ...
        "assignment_plans": {
            "developer": {
                "zone": "z",
                "runtimes": {
                    "claude": {"profiles": ["p"], "model": "claude-x", "effort": "high"}
                },
            },
        },
    }
...
class ResolveAssignmentTests(unittest.TestCase):
    ...
    def test_write_paths_outside_zone(self) -> None:
        ...
        self.assertEqual(
            error.message,
            "role 'developer' write_paths must remain inside backend zone 'z'",
        )
    ...
    def test_runtime_entry_not_an_object(self) -> None:
        ...
        error = self._fail(config, _role())
        self.assertEqual(
            error.message, "role 'developer' is not assigned to runtime 'claude'"
        )
    ...
    def test_incompatible_profile(self) -> None:
        ...
        error = self._fail(config, _role())
        self.assertEqual(
            error.message, "provider profile 'p' is incompatible with role 'developer'"
        )
    ...
    def test_invalid_role_manifest_name(self) -> None:
        ...
        self.assertEqual(
            error.message, "role manifest 'developer' has invalid name or mode"
        )
    ...
    def test_invalid_model_and_effort(self) -> None:
        ...
        self.assertEqual(
            self._fail(config, _role()).message,
            "assignment model must be a CLI model ID or alias without spaces",
        )
        config = _config()
        config["assignment_plans"]["developer"]["runtimes"]["claude"]["effort"] = (
            "turbo"
        )
        self.assertTrue(
            self._fail(config, _role()).message.startswith(
                "assignment effort must be one of:"
            )
        )


class PolicyProblemTests(unittest.TestCase):
    FIELDS: ClassVar[set[str]] = {"a", "b"}
    ...
    def test_non_object(self) -> None:
        self.assertEqual(
            contract._policy_problem({"p": 1}, "p", self.FIELDS),
            ["orchestration p must be an object"],
        )
    ...
    def test_bool_and_low_values_are_not_positive_integers(self) -> None:
        problems = contract._policy_problem(
            {"p": {"a": True, "b": 0}}, "p", self.FIELDS
        )
        self.assertEqual(
            sorted(problems),
            [
                "orchestration p.a must be a positive integer",
                "orchestration p.b must be a positive integer",
            ],
        )

    def test_minimum_zero_allows_zero(self) -> None:
        self.assertEqual(
            contract._policy_problem({"p": {"a": 0}}, "p", self.FIELDS, minimum=0), []
        )
        self.assertEqual(
    ...
    def test_boolean_fields(self) -> None:
        self.assertEqual(
            contract._policy_problem(
                {"p": {"flag": True}}, "p", self.FIELDS, booleans={"flag"}
            ),
            [],
        )
        self.assertEqual(
            contract._policy_problem(
                {"p": {"flag": 1}}, "p", self.FIELDS, booleans={"flag"}
            ),
