...
def check_pvmalove_additions_docs_sync() -> None:
    ...
    capabilities = json.loads((ROOT / "harness" / "CAPABILITIES.json").read_text(encoding="utf-8"))
    expected = {
        entry.removeprefix("first-party/pvmalove/") for entry in capabilities["pvmalove-suite"]["additions"]
    }
...
ALWAYS_SENT_INSTRUCTION_FILES = (
    [ROOT / "CLAUDE.md", ROOT / "AGENTS.md", ROOT / "harness" / "orchestration" / "playbook.md"]
