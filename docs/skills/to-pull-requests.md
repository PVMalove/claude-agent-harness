...
4. Подготовьте тело PR по `docs/agents/git-workflow.md` §3. Храните его только в `.harness/scratch/tmp/pr-body-<issue>-<slug>.md`, никогда в `docs/tasks/`. Разработчик может вручную запустить `pr-composer` в coding application; иначе заполните шаблон напрямую. Разрешите default branch репозитория и используйте `Closes #<ID>` только для этой цели, иначе `Related to #<ID>`.
5. Попросите разработчика явно подтвердить, что ветка готова стать PR. Остановитесь для его ответа.
6. После одобрения откройте PR/MR через CLI tracker-а с `--body-file <path>`. Удаляйте файл тела `.harness/scratch/tmp/` только после успеха этой команды; при ошибке сохраните его для повторной попытки. Если тикет несёт `task-report::required`, опубликуйте его completion report в тикете, если разработчик не попросил пропустить это.
