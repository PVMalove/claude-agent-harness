...
3. Для действительного opt-in `backend-orchestration` после синхронизации разрешите полный текущий SHA через `git rev-parse HEAD` и проверьте его до любого действия с PR:
   ...
   Остановитесь, если команда отклоняет отсутствующие, непринятые или не совпадающие с SHA доказательства. Не запускайте `/qa-gate` повторно, когда эта проверка успешна. Если проект не является действительным opt-in, запустите `/qa-gate`, если этот репозиторий его предоставляет, и остановитесь при ошибке.
