...
The system-wide work model, including coordinator-managed backend batches, is described in
[current-state.md](./current-state.md). This guide covers user-managed parallel worktrees only.
...
- **Managed harness worktrees**: create coordinator and CLI-owned issue checkouts under the main checkout's `.harness/worktrees/`. Preview stale clean worktrees with `harness cleanup <repo> --mode hard`; `--apply --confirm HARD` removes eligible directories and their local branches. Ledger-referenced, dirty, current, and unpushed worktrees stay in place. Remote branches stay in place.
