...
* **PR bodies:** A PR body or comment is one-shot publication metadata, not a task artifact. Save it only under `.scratch/tmp/` (for example, `.scratch/tmp/pr-body-<issue>-<slug>.md`), never in `docs/tasks/`; delete it after the `gh`/`glab` command succeeds and keep it when the command fails so it can be retried.
* **Temporary Solutions & Prototypes:** For short-lived exploration, generated HTML prototypes, or temporary scripts (like a discovery trail), save them in `.scratch/tmp/`. The commit hook intentionally permits writing to `.scratch/tmp/` to distinguish them from system temp directories.
