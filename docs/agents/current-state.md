...
python .harness/orchestration/coordinator.py --repo . dispatch create ... --transition-digest <digest>
...
`dispatch propose` показывает канонический переход и его digest, а `dispatch create` с явным approval
принимает только этот digest: approval действует ровно для показанного перехода. Флаг
`needs_attention` останавливает создание следующего dispatch до решения человека
