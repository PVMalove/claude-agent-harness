...
**Оркестрация sub-agent'ов (MUST):** это правило про то, *как* делегировать саб-агенту, когда координатор уже решил делегировать — не про то, что делегировать нужно любую задачу. Подавляющее большинство шагов координатор выполняет сам в текущей сессии, без спавна кого-либо; заворачивать в оркестратор всё подряд — это трата токенов и времени, а не добросовестное исполнение правила. Если сессия работает под Orca (`TERM_PROGRAM=Orca` / `ORCA_WORKSPACE_ID` в окружении) и координатор всё же делегирует саб-агенту с отдельным deliverable (тело PR, код, отчёт), такой спавн **обязан** идти через Orca Run → Task → Dispatch, а координатор ждёт `worker_done` через `orca orchestration check --wait`, обрабатывает доставленные результаты и продолжает workflow автоматически. Прямой generic agent spawn обходит lifecycle Orca и **не используется** для deliverable-задач (допустим только для тривиального in-session lookup, например `Explore`). Недостаточное исполнение правила — координатор не видит завершения сабагента, workflow не продолжается сам, пользователю приходится вручную инициировать каждый следующий шаг. Избыточное исполнение — тоже нарушение: делегирование через оркестратор задач, которые не требовали отдельного саб-агента. `--agent` у `worker-start` обязателен и явный — Orca не подставляет `claude`/`claude-code` сама; координатор указывает `--agent`, совпадающий с его собственным рантаймом (Codex-координатор запускает `--agent codex`-воркера), кроме случаев, когда пользователь явно просит воркера другого семейства. Файлы `.claude/agents/*.md` (в т.ч. `pr-composer`, раздел 6) — это markdown-спецификация задачи, а не Claude-Code-специфичный вызов: их может выполнить воркер любого рантайма, читая файл как обычную инструкцию. В обычном рантайме (без Orca) действует fallback с уведомлениями harness.
