...
Обычно этот раздел не нужно проходить руками — достаточно сказать агенту «используй `start-project`, чтобы помочь мне сформировать и начать этот проект» (см. README, раздел «Быстрый старт»), и он сам выберет capability и вызовет нужную команду `harness` от вашего имени. Ниже — что происходит под капотом, и как сделать то же самое напрямую через CLI.
