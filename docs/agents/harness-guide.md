...
- **Как работает:**
  ...
  4. **В этом репозитории** (локальная кастомизация, раздел 7): пишет спеку сначала файлом в `docs/tasks/`, публикует через `gh issue create --body-file`; помечает `workflow::specs` (не `workflow::ready` — эпик ещё не декомпозирован) — `/to-tickets` линкует дочерние тикеты к этому issue как native GitHub sub-issues, а не через лейбл (см. `docs/agents/triage-labels.md`).
...
- **Как работает:**
  ...
  4. **В этом репозитории**: если исходный issue — эпик (`workflow::specs`), каждый дочерний тикет линкуется к нему как native GitHub sub-issue (не лейблом); тикет, заблокированный другим ещё не закрытым тикетом той же декомпозиции, получает `workflow::blocked` вместо `workflow::ready` — фронтир (какие тикеты уже разблокированы) ищется тем же native-запросом, что использует `wayfinder`, а не перебором вручную; `/implement`, если вызван на сам эпик, сам находит и клеймит первый тикет фронтира (раздел 7, ADR в `docs/adr/`).
  5. По завершении публикации выводит итоговую таблицу тикетов с кратким описанием того, что нужно сделать — описания генерирует не основная модель сессии, а дешёвая (`haiku`), одним вызовом на весь пакет тикетов.
...
- **Как работает:**
  ...
  2. Если вызван со ссылкой на тикет (номер/URL/путь к локальному файлу) и тот несёт `workflow::blocked` — сначала проверяет блокеры и либо переводит в `workflow::ready`, либо отказывается стартовать, называя открытые блокеры (раздел 7). Если вызван со ссылкой на сам эпик, а не на конкретный тикет — сам находит первый разблокированный тикет декомпозиции (native frontier query) и клеймит его (`--add-assignee @me`) прежде чем начать, чтобы не столкнуться с параллельной сессией в другом worktree. Начиная работу, ставит `workflow::in-progress`.
  3. Строит код через `/tdd` (раздел 5), затем прогоняет `/code-review`.
  4. Коммитит на текущую (feature-)ветку.
  5. **В этом репозитории** (локальная кастомизация, раздел 7): следует `docs/agents/git-workflow.md` для открытия PR — запускает `qa-gate` перед `gh pr create`, делегирует тело PR субагенту `pr-composer` (раздел 6), затем останавливается и ждёт решения разработчика — ревью или доработки. Никогда не мерджит сам. Если тикет несёт `task-report::required`, при открытии PR публикует отчёт о завершении комментарием на issue. Для локального (не-GitHub/GitLab) трекера, где нет закрытия через мердж, по завершении ставит тикету `**Workflow:** done`.
...
- **Назначение:** `hitl`-аналог `/implement` — вместо того чтобы писать код самому, готовит человеку «штурманский лист»: карту файлов и готовые промпты для AI IDE (Cursor, Copilot Chat), по одному тикету.
- **Как работает:** читает тикет (ожидает `hitl` + `workflow::ready`), сканирует кодовую базу под File Map, клеймит тикет (`--add-assignee @me`, тот же паттерн, что у `wayfinder`/`implement`), переводит его в `workflow::in-progress`, пишет гайд в `docs/tasks/` (в папку эпика, если тикет из декомпозиции — `docs/agents/artifacts.md`).
- **Дальше — полностью вручную:** кодинг, `qa-gate`, PR по `docs/agents/git-workflow.md` — этот скилл их не делает и не вызывается повторно для того же тикета.
